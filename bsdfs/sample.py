...
class MyBSDF(mi.BSDF):
    ...
    def sample(self, ctx, si, sample1, sample2, active):
        ...
        bs.sampled_component = dr.select(
            selected_r, mi.UInt32(0), mi.UInt32(1))
        ...
        value_r = dr.lerp(mi.Color3f(self.tint), mi.Color3f(
            1.0), dr.clamp(cos_theta_i, 0.0, 1.0))
...
scene: mi.Scene = mi.load_file(
    RESOURCE_DIR+'extra/cbox-custombsdf.xml')

image = mi.render(scene, spp=50)

mi.util.write_bitmap('custom.exr', image)
